    ...
    <div className="py-8 bg-black text-white">
      ...
      <Slider {...settings}>
        {events.map((event, index) => (
          <div
            key={index}
            className={`event-card ${index === getMiddleSlideIndex(currentSlide) ? 'active' : ''}`}
          >
            <div className="bg-black p-6 rounded-lg border border-yellow-500 relative">
              <img
                src={event.image}
                alt={event.title}
              ...
              <a
                href="#"
                className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-yellow-500 text-black px-4 py-2 rounded"
              >
                View More
              </a>
            </div>
