...
import MemberCard25 from "@/components/memberCard25";
            ...
            <div className="mb-20">
              {execom.map((item, index) => {
                const filteredMembers = teamData.filter(
                  (member) => member[2].toLocaleLowerCase() === item.role
                );
                const gridColsClass =
                  filteredMembers.length === 1
                    ...
                    ? "grid-cols-2 md:grid-cols-4 md:gap-32"
                    : "md:grid-cols-3 grid-cols-2 md:gap-32";
                  ...
                  >
                    ...
                    >
                      {filteredMembers.map((member, index) => (
                        <Link href={`/team/${member[3].split("/").pop()}`} key={index}>
                          <MemberCard25 data={member} />
                        </Link>
                      ))}
                    </div>
                  </div>
